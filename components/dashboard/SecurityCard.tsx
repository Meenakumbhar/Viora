...
export default function SecurityCard() {
  ...
  return (
    <div className="border border-border p-6 sm:p-7">
      <p className="font-mono text-sm uppercase tracking-[0.18em] text-text-heading">Security</p>
      ...
      <div className="mt-5 border-t border-dashed border-border pt-5">
        {changingPassword ? (
          <div className="space-y-4">
            <div>
              <label htmlFor="current-password" className="block font-mono text-sm uppercase tracking-widest text-text-muted">
                Current password
              </label>
              <input
                id="current-password"
            ...
            <div>
              <label htmlFor="new-password" className="block font-mono text-sm uppercase tracking-widest text-text-muted">
                New password
              </label>
              <input
                id="new-password"
                ...
                className="mt-1.5 w-full border border-border bg-bg-primary px-3 py-2.5 font-body text-sm text-text-heading outline-none transition-colors focus:border-accent-gold"
              />
              <p className="mt-1.5 font-mono text-sm text-text-muted">At least 8 characters.</p>
            </div>
            <div>
              <label htmlFor="confirm-new-password" className="block font-mono text-sm uppercase tracking-widest text-text-muted">
          ...
          <div className="flex items-center justify-between">
            <p className="font-body text-sm text-text-heading">Password</p>
            <button
              type="button"
      ...
      <div className="mt-5 border-t border-dashed border-border pt-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="font-body text-sm text-text-heading">Other devices</p>
          ...
          <button
            ...
            disabled={revoking}
            className="border border-border px-4 py-2 font-mono text-sm uppercase tracking-widest text-text-heading transition-colors hover:border-accent-gold hover:text-accent-gold disabled:opacity-50"
          >
            {revoking ? 'Signing out…' : 'Sign out other devices'}
          </button>
        </div>
        {revokeMessage && <p className="mt-3 font-mono text-sm text-text-muted">{revokeMessage}</p>}
      </div>
    </div>
  );
}
